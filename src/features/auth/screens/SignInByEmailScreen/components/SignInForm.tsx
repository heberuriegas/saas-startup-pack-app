...
const SignInForm: React.FC = () => {
  ...
  const [githubSignInIsLoading, setGithubSignInIsLoading] = React.useState<boolean>(false);
  const { signInByEmail, githubSignIn, googleSignIn } = useContext(AuthContext);
  const toast = useThemedToast();
  const linkTo = useLinkTo();

  const anySignInIsLoading = useMemo(
    () => signInIsLoading || googleSignInIsLoading || githubSignInIsLoading,
    [signInIsLoading, googleSignInIsLoading, githubSignInIsLoading]
  );
  ...
  const onGithubSignInPress = async (): Promise<void> => {
    setGithubSignInIsLoading(true);
    try {
      if (githubSignIn) await githubSignIn();
    } catch (err: any) {
      Sentry.captureMessage(err);
      toast.show({ description: 'We are sorry, something went wrong' });
    } finally {
      setGithubSignInIsLoading(false);
    }
  };
      ...
      <VStack
        ...
        <VStack space="7">
          ...
          <VStack>
            ...
            {/* <Center>
              <HStack space="4">
                <Pressable disabled={anySignInIsLoading} onPress={onGithubSignInPress}>
                  {githubSignInIsLoading ? <Spinner /> : <Octicons name="mark-github" size={24} />}
                </Pressable>
