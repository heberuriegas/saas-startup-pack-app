...
const getCredentialsFromHeaders: GetCredentialsFromHeaders = (headers: AxiosResponseHeaders) => {
...
export const signUpByEmailApi: SignUpByEmailApi = async (user) => {
  ...
  >(
    ...
    {
      headers: {
        'Client-Id': Config.AUTH_CLIENT_ID,
      },
    }
  );
...
export type SignInByPhoneNumberApiParams = SignInByPhoneNumberParams;
...
export const meApi: MeApi = async () => {
  ...
  if (credentials) {
    ...
    if (result.data.me) return result.data.me;
  }
  return null;
};
